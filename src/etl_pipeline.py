...
def clean_price_column(df):
    ...
    logger.warning(f"Found {bad_price_count} orders with suspicious price")
...
def remove_test_data(df):
    ...
    df_filtered = df.filter(
        ~(upper(col("customer_id")).contains("TEST") |
          ...
          upper(col("region")).contains("TEST") |
...
def transform_orders(df):
    ...
    df = df.withColumn("total_amount", 
                       col("price_decimal")*col("quantity")) \
    ...
    logger.info(f"Transformations complte. Final record count: {df.count()}")
...
def load_to_csv(spark, df, output_path):
    ...
    # Convert to pandas for local development ONLY 9not suitable for large datasets)
...
def create_summary_report(df):
    ...
    summary = {
        ...
        "total_revenue": df.agg(sum(col("total_amount"))).collect()[0][0],
